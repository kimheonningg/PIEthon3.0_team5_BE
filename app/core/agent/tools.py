...
def create_tools_with_user_context(db, user, patient_mrn: Optional[str] = None) -> List:
    ...
    async def get_recent_notes_with_context(
        ...
        # Handle FieldInfo objects passed by LangChain
         
        if isinstance(limit, FieldInfo):
            limit = 5
        if isinstance(note_type, FieldInfo):
            note_type = None
    ...
    async def get_recent_appointments_with_context(
        ...
        if isinstance(limit, FieldInfo):
            limit = 5
        if isinstance(future_only, FieldInfo):
            future_only = False
    ...
    async def get_recent_medical_histories_with_context(
        ...
        if isinstance(limit, FieldInfo):
            limit = 5
        if isinstance(tag_filter, FieldInfo):
            tag_filter = None
    ...
    async def get_recent_lab_results_with_context(
        ...
        if isinstance(limit, FieldInfo):
            limit = 10
        if isinstance(test_name_filter, FieldInfo):
